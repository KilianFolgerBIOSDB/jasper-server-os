/*
 * Adapted from org.springframework.web.servlet.view.tiles3.TilesConfigurer
 * from Spring Framework version 5.3.26
 ...
 */
public class JasperTilesConfigurer implements ServletContextAware, InitializingBean, DisposableBean {

	private static final boolean tilesElPresent =
			ClassUtils.isPresent("org.apache.tiles.el.ELAttributeEvaluator", JasperTilesConfigurer.class.getClassLoader());
	...
	public void setCompleteAutoload(boolean completeAutoload) {
		if (completeAutoload) {
			try {
				this.tilesInitializer = new JasperCompleteAutoloadTilesInitializer();
			}
			catch (Throwable ex) {
	...
	public void afterPropertiesSet() throws TilesException {
		...
		ApplicationContext preliminaryContext = new JasperWildcardServletTilesApplicationContext(this.servletContext);
	...
	private class SpringTilesContainerFactory extends BasicTilesContainerFactory {
		...
		protected LocaleResolver createLocaleResolver(ApplicationContext context) {
			return new JasperLocaleResolver();
		}
	...
	private static class JasperCompleteAutoloadTilesInitializer extends CompleteAutoloadTilesInitializer {

		@Override
		protected AbstractTilesContainerFactory createContainerFactory(ApplicationContext context) {
			return new JasperCompleteAutoloadTilesContainerFactory();
		}
	}


	private static class JasperCompleteAutoloadTilesContainerFactory extends CompleteAutoloadTilesContainerFactory {
